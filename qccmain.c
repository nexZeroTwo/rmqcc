...
//	{"", WARN_CASEINSENSATIVEFRAMEMACRO},
	...
	{"F300", WARN_DEADCODE},
	{"RM00", WARN_DEPRECATEDTERNARY},
	{"RM01", WARN_DEPRECATEDLOCAL},
	{"RM02", WARN_DEPRECATEDFUNCTIONSTYLE},
	{NULL}
};
...
compiler_flag_t compiler_flag[] = {
	...
	{&flag_fixternary,		FLAG_MIDCOMPILE,"fixternary",	"Fix ?: operator",		"Fixes the ?: operator to behave as in C, and disables its deprecation warning. This may break some weird/bad code, but usually should be safe to turn on."},
...
//			numfunctions++;
		...
		else if ((def->scope||def->constant) && (def->type->type != ev_string || (opt_constant_names_strings && strncmp(def->name, "dotranslate_", 12))))
		{
